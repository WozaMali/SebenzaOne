...
// Adapted for CRM app - email service integration is optional

import { supabase } from './supabase-client'
import { crmService } from './crm-service'

// Check if email service is available (optional dependency)
let emailService: any = null
try {
  // Try to import email service if available
  if (typeof window !== 'undefined') {
    // Email service would be imported here if available
    // For now, we'll make it optional
  }
} catch (e) {
  // Email service not available, that's okay
}
...
class IntegrationService {
  ...
  constructor() {
    // Don't initialize during module load - wait for explicit initialization
  }
  ...
  private async findContactByEmail(email: string) {
    try {
      ...
      return contacts.find((c: any) => c.email.toLowerCase() === email.toLowerCase())
    } catch (error) {
  ...
  private async syncEmailData() {
    try {
      // Sync email data with other modules (only if email service is available)
      if (!emailService) {
        console.log('Integration: Email service not available, skipping email sync')
        return
      }
  ...
  async getCrossModuleData(): Promise<CrossModuleData> {
    ...
    try {
      ...
      // Get emails if email service is available
      let emails: any[] = []
      if (emailService && emailService.getEmailsForFolder) {
        try {
          emails = emailService.getEmailsForFolder('inbox') || []
        } catch (e) {
          console.warn('Could not get emails:', e)
        }
      }
  ...
  async searchAcrossModules(query: string) {
    ...
    try {
      ...
      const results = {
        ...
        companies: data.companies.filter((c: any) => 
          c.name?.toLowerCase().includes(query.toLowerCase()) ||
          c.industry?.toLowerCase().includes(query.toLowerCase())
        ),
        deals: data.deals.filter((d: any) => 
          d.name?.toLowerCase().includes(query.toLowerCase()) ||
          d.description?.toLowerCase().includes(query.toLowerCase())
        ),
        emails: data.emails.filter((e: any) => 
          e.subject?.toLowerCase().includes(query.toLowerCase()) ||
          e.body?.toLowerCase().includes(query.toLowerCase())
        ),
        activities: data.activities.filter((a: any) => 
